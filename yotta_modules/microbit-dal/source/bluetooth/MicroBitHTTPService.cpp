...
MicroBitHTTPService::MicroBitHTTPService(BLEDevice &_ble) : ble(_ble) {
  ...
  GattCharacteristic requestCharacteristic(MicroBitHTTPServiceRequestUUID, (uint8_t*)&requestCharacteristicBuffer, 20, 20, GattCharacteristic::BLE_GATT_CHAR_PROPERTIES_READ | GattCharacteristic::BLE_GATT_CHAR_PROPERTIES_WRITE);
...
void MicroBitHTTPService::updateURL(ManagedString url) {
  ...
  ble.gattServer().write(urlCharacteristicHandle, urlBytes, sizeof(urlBytes));
}

uint8_t* MicroBitHTTPService::requestHTTP(HTTP_TYPE type, char* field) {
  char* requestMessage = (char*)malloc(20); 
  switch(type) {
    case HTTP_GET:  
      requestMessage[0] = 'G';
      requestMessage[1] = '\0';
